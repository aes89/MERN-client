...
import {deleteFridgeItem, deletePantryItem, getFridge, setFridge, getPantry, setPantry} from '../../services/ingredientServices'
...
const Ingredient = ({ actions, recipe, ingredients, deleteIngredient, fridgeIngredients, pantryIngredients }) => {
...
const handleDeleteFromFridge = async (event) => {
      console.log("deleted 1 Item fridge");
       ...
       // console.log(newArrayy)
        deleteFridgeItem(getUsername(),{ item: event.currentTarget.value }).then((r) => {
              console.log(r) 
              //setFridge(newArrayy)
...
const handleDeleteFromPantry = async (event) => {
        console.log("deleted 1 Item pantry");
         ...
         deletePantryItem(getUsername(), { item: event.currentTarget.value }).then((r) => {
              console.log(r)
          ...
          {ingredients.map((ingredient) => (
            <Paper className={classes.paperIngredient} variant="outlined">
              <div class={styles.ingredientText}>
                <p>{ingredient} </p>
                <Button value={ingredient} onClick={ingredients=== fridgeIngredients ? handleDeleteFromFridge : handleDeleteFromPantry}>
                <ClearIcon />
                </Button>
              </div>
            </Paper>
